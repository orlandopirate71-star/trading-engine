"""
VWAP Rejection Strategy - Mean reversion using VWAP with volume confirmation.
...
- Volume confirms rejection strength
"""
import sys
import os
...
class VWAPRejectionStrategy(BaseStrategy):
    ...
    def _check_volume_confirmation(self, symbol: str, volume_threshold: float = 1.5) -> tuple:
        """
        Check if recent volume supports a move.
        Returns (confirmed, avg_volume, current_volume).
        For this strategy we use estimated volume since OANDA may not provide real volume.
        """
        # OANDA doesn't provide real-time volume in their tradebook API
        # So we use a simulated volume based on price movement strength
        if len(self.recent_prices) < 20:
            return False, 1.0, 1.0

        # Simulate volume: larger price movements = higher "volume"
        recent = self.recent_prices[-20:]
        price_changes = [abs(recent[i] - recent[i-1]) / recent[i-1] for i in range(1, len(recent))]
        avg_change = sum(price_changes) / len(price_changes) if price_changes else 0.0001
        current_change = price_changes[-1] if price_changes else 0

        # Volume is "confirmed" if current movement is stronger than average
        ratio = current_change / avg_change if avg_change > 0 else 1.0

        return ratio > volume_threshold, avg_change, current_change
    ...
    def on_tick(self, symbol: str, price: float, timestamp: datetime) -> Optional[TradeSignal]:
        ...
        volume_ok, _, _ = self._check_volume_confirmation(symbol, volume_threshold=self.volume_threshold)

        # Long: uptrend + touched VWAP from above + bullish rejection + volume
        if uptrend and self.touched_vwap and self.touch_side == "above" and self._is_bullish_rejection() and volume_ok:
            ...
            return self.create_signal(
                ...
                confidence=0.75,
                reason=f"Bullish VWAP rejection, slope={slope*100:.3f}%, volume confirmed"
            )

        # Short: downtrend + touched VWAP from below + bearish rejection + volume
        if downtrend and self.touched_vwap and self.touch_side == "below" and self._is_bearish_rejection() and volume_ok:
            self.ticks_since_signal = 0
            self.touched_vwap = False
            ...
            return self.create_signal(
                ...
                take_profit=take_profit,
                confidence=0.75,
                reason=f"Bearish VWAP rejection, slope={slope*100:.3f}%, volume confirmed"
            )
