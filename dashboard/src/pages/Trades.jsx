    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap gap-4 bg-gray-800 p-4 rounded-lg border border-gray-700">
        ...
        <select
          ...
          className="bg-gray-700 border border-gray-600 rounded px-3 py-1.5 text-sm text-white cursor-pointer"
        >
          ...
          <table className="w-full">
            <thead className="bg-gray-900">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">Closed By</th>
              ...
              {loading ? (
                ...
                trades.map((trade) => (
                  <tr key={trade.id} className="hover:bg-gray-750">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-400">{trade.strategy_name}</td>
                    ...
                    <td className="px-4 py-3">
                      {trade.pnl !== null && trade.pnl !== 0 ? (
                        <div className={`font-mono font-medium ${
                          trade.is_win ? 'text-green-400' : 'text-red-400'
                        }`}>
                          {trade.is_win ? '+' : ''}{trade.pnl?.toFixed(2)}
                          <div className="text-xs text-gray-500">
                            {trade.pnl_percent?.toFixed(3)}%
                          </div>
                        </div>
                      ) : trade.status === 'open' ? (
                    ...
                    <td className="px-4 py-3">
                      <Link 
                        to={`/trades/${trade.id}`}
                        className="p-2 hover:bg-gray-700 rounded transition-colors inline-block"
                      >
